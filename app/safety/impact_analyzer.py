...
from app.safety.models import CommandType, ImpactAnalysis, RiskLevel
...
class ImpactAnalyzer:
    ...
    def __init__(self):
        ...
        self.resource_patterns = {
            "gcp_project": re.compile(
                r"(?:projects?[/=]\s*|--project[= ]|binding\s+)([a-z0-9][-a-z0-9]{4,28}[a-z0-9])",
                re.IGNORECASE,
            ),
            ...
            "service_account": re.compile(
                r"serviceAccount:([^'\"]+@[^'\"]+\.iam\.gserviceaccount\.com)", re.IGNORECASE
            ),
            "iam_role": re.compile(r"roles?/([a-zA-Z0-9\._]+)", re.IGNORECASE),
            "firewall_port": re.compile(r"port[= ](\d+)", re.IGNORECASE),
            "system_service": re.compile(
                r"(nginx|apache|mysql|postgresql|redis|docker)", re.IGNORECASE
            ),
        }

    def analyze_impact(
        self, command: str, command_type: CommandType, risk_level: RiskLevel
    ) -> ImpactAnalysis:
        ...
        rollback_cmd = (
            self._generate_rollback_command(command, command_type) if reversible else None
        )
    ...
    def _generate_rollback_command(self, command: str, _command_type: CommandType) -> Optional[str]:
        """Generate a rollback command if possible."""
        rollback_mappings = {
            # Firewall rules
            r"firewall-cmd.*--remove-port[= ](\d+/\w+)": (
                r"firewall-cmd --permanent --add-port=\1"
            ),
            r"firewall-cmd.*--remove-service[= ](\w+)": (
                r"firewall-cmd --permanent --add-service=\1"
            ),
            # IAM bindings
            (
            ...
            # File permissions
            r"chmod (\d+) (.+)": (
                r"chmod [ORIGINAL_PERMISSIONS] \2  "
                r"# Note: Original permissions need to be determined"
            ),
        }
