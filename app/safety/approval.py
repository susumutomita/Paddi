...
from app.safety.models import (
    ApprovalRequest, ApprovalStatus, CommandValidation,
...
class ApprovalWorkflow:
    ...
    def __init__(self):
        ...
        self.auto_approval_rules = {
            'low_risk_readonly': lambda v, i: v.risk_level == RiskLevel.LOW and not i.data_loss_risk,
    ...
    def _check_auto_approval(self, validation: CommandValidation, impact: ImpactAnalysis) -> ApprovalStatus:
    ...
    def reject_request(self, request_id: str, rejector: str, reason: str) -> Optional[ApprovalRequest]:
    ...
    def format_approval_request(self, request: ApprovalRequest) -> str:
        ...
        lines = [
            ...
            f"- Command Type: {request.validation.command_type.value}",
        ]
        ...
        if len(request.impact_analysis.affected_resources) > 5:
            lines.append(f"  • ... and {len(request.impact_analysis.affected_resources) - 5} more")
        ...
        if request.status != ApprovalStatus.PENDING:
            lines.extend([
                ...
                f"- Decided at: {request.approved_at.strftime('%Y-%m-%d %H:%M:%S UTC') if request.approved_at else 'N/A'}",
            ])
