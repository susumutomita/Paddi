...
from app.safety.models import (
    AuditLogEntry, CommandValidation, ImpactAnalysis,
    ApprovalRequest
)
...
class AuditLogger:
    ...
    def generate_audit_report(self, start_time: datetime, end_time: datetime) -> str:
        ...
        stats = {
            ...
            'auto_approved': sum(1 for e in entries if e.approval and e.approval.status.value == 'auto_approved'),
        }
        ...
        high_risk_commands = [
            e for e in entries
            if e.validation_result.risk_level.value in ['high', 'critical']
        ]
    ...
    def export_logs(self, output_file: Path, format: str = "json") -> None:
        ...
        if format == "json":
            ...
            with open(output_file, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([
                    'timestamp', 'id', 'user', 'command', 'risk_level',
                    'command_type', 'dry_run', 'approved', 'error'
                ])
