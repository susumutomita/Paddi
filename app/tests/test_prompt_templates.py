...
class TestPromptTemplates:
    ...
    def test_enhanced_security_analysis_prompt(self):
        ...
        assert (
            "クラウドセキュリティとアプリケーションセキュリティの専門家"
            in ENHANCED_SECURITY_ANALYSIS_PROMPT
        )
    ...
    def test_get_enhanced_prompt_security_analysis(self):
        ...
        data = {"infrastructure_data": "IAM policies", "application_data": "Dependencies"}
        ...
        # Check no unfilled placeholders (but allow JSON format which has braces)
        import re

        # Look for template placeholders like {variable_name}
        unfilled = re.findall(r"\{[a-z_]+\}", prompt)
        assert len(unfilled) == 0, f"Found unfilled placeholders: {unfilled}"
    ...
    def test_get_enhanced_prompt_dependency_analysis(self):
        ...
        data = {"vulnerability_data": "CVE list"}
    ...
    def test_get_enhanced_prompt_multi_cloud(self):
        ...
        data = {"cloud_providers": "AWS, GCP, Azure", "security_configs": "Security settings"}
    ...
    def test_build_analysis_prompt_empty_findings(self):
        ...
        context = {"project_name": "test-app", "environment": "development"}
