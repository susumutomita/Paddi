...
class TestSafetyCheck:
    ...
    def test_validate_critical_risk_command(self, safety_check):
        ...
        is_safe, message, approval = safety_check.validate_command(
            command="firewall-cmd --permanent --remove-port=443/tcp", user="test-user", dry_run=True
        )
    ...
    def test_validate_with_dry_run_output(self, safety_check):
        ...
        _, message, _ = safety_check.validate_command(
            command="systemctl stop nginx", user="test-user", dry_run=True
        )
    ...
    def test_validate_with_force_approval(self, safety_check):
        ...
        is_safe, _, approval = safety_check.validate_command(
            command="echo 'Hello World'", user="test-user", dry_run=True, force_approval=True
        )

        # When force_approval=True, the command may be safe but still require approval
        assert approval is not None
        assert approval.status == ApprovalStatus.PENDING
        # is_safe should be False because approval is pending
        assert not is_safe

    def test_execute_command_dry_run(self, safety_check):
        """Test executing command in dry-run mode."""
        # Use a safe command that doesn't require approval
        success, result = safety_check.execute_command(
            command="ls -la /tmp", user="test-user", dry_run=True
        )
    ...
    def test_execute_command_requiring_approval(self, safety_check):
        ...
        success, result = safety_check.execute_command(
            command="gsutil rm -r gs://production-bucket/", user="test-user", dry_run=False
        )
    ...
    def test_execute_command_with_approval(self, safety_check):
        ...
        _, _, approval = safety_check.validate_command(command=command, user="test-user")
    ...
    def test_reject_command(self, safety_check):
        ...
        rejected_request = safety_check.reject_command(
            approval.id, "admin-user", "Too dangerous for production"
        )
    ...
    def test_get_pending_approvals(self, safety_check):
        ...
        pending_ids = []
        for cmd in commands:
            _, _, approval = safety_check.validate_command(command=cmd, user="test-user")
    ...
    def test_get_audit_report(self, safety_check):
        ...
        for cmd, user in commands:
            safety_check.execute_command(command=cmd, user=user, dry_run=True)
    ...
    def test_search_audit_logs(self, safety_check):
        ...
        # Create some activity
        safety_check.execute_command(command="gcloud projects list", user="alice", dry_run=True)

        safety_check.execute_command(command="rm -rf /data", user="bob", dry_run=True)
        ...
        # Search by risk level (rm -rf is critical risk)
        critical_risk_logs = safety_check.search_audit_logs(risk_level="critical")
        assert len(critical_risk_logs) >= 1
        assert all(
            log["validation_result"]["risk_level"] == "critical" for log in critical_risk_logs
        )
    ...
    def test_integration_workflow(self, safety_check):
        ...
        is_safe, message, approval = safety_check.validate_command(
            command=command, user=user, dry_run=True
        )
        ...
        _ = [log for log in logs if log["command"] == command and not log["dry_run"]]
