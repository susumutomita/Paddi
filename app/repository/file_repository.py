...
class FileRepository(Repository):
    ...
    def _get_file_path(self, key: str, output_format: str = "json") -> Path:
    ...
    def save(self, key: str, data: Any, output_format: str = "json", **kwargs) -> None:
        """Save data to file.

        Args:
            key: Key for the data
            data: Data to save
            output_format: File format ('json' or 'text')
        """
        if output_format not in ["json", "text"]:
            raise ValueError(f"Unsupported format: {output_format}")

        file_path = self._get_file_path(key, output_format)
    ...
    def load(self, key: str, output_format: Optional[str] = None, **kwargs) -> Optional[Any]:
        """Load data from file.

        Args:
            key: Key for the data
            output_format: File format (auto-detected if None)
        """
        if output_format is None:
            output_format = self._detect_format(key)
