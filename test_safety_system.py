...
from app.main import PaddiCLI  # noqa: E402
from app.safety.models import ApprovalStatus, RiskLevel  # noqa: E402
...
def test_safety_system():
    ...
    is_safe, message, approval = cli.safety_check.validate_command(
        command=command, user="test-user", dry_run=True
    )
    ...
    if approval_request and approval_request.status == ApprovalStatus.PENDING:
        # Approve it
        approved = cli.safety_check.approve_command(approval_request.id, "admin-user")
