...
resource "google_pubsub_topic" "destroy_trigger" {
  name = "paddi-hackathon-destroy-trigger"
}
...
# 自動削除ジョブ（ハッカソン環境は常に有効）
resource "google_cloud_scheduler_job" "destroy_hackathon" {
  name        = "destroy-hackathon-env"
  description = "ハッカソン環境を自動削除（コスト削減）"
  schedule    = "0 6 * * *" # 毎日午前6時に削除
