...
enum ConfigCommands {
    ...
    Init {
        #[arg(long, help = "Output path for config file", default_value = "paddi.toml")]
        output: PathBuf,
...
async fn validate_config(file: Option<PathBuf>, default_config: Config) -> Result<()> {
    ...
    println!("\n🔍 Checking agents path: {}", config.python.agents_path.display());
    ...
    // Show other settings
    println!("\n📋 Other settings:");
    println!("   GCP Project ID: {}", config.gcp.project_id.as_deref().unwrap_or("Not set"));
